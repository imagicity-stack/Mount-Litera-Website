  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="font-garamond text-2xl font-semibold text-midnight">Popups</h2>
          <p className="text-sm text-midnight/55">Design, target, and measure on-site campaigns.</p>
        </div>
      ...
      {loading ? (
        <div className="rounded-2xl border border-midnight/10 bg-white px-5 py-10 text-center text-sm text-midnight/50 shadow-elite-sm">Loading popups…</div>
      ) : visible.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-midnight/20 px-5 py-12 text-center text-sm text-midnight/50">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {visible.map((popup) => {
            ...
            return (
              <div key={popup.id} className="flex flex-col rounded-2xl border border-midnight/10 bg-white p-5 shadow-elite-sm">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="truncate font-semibold text-midnight">{popup.name || 'Untitled'}</p>
                    <p className="truncate text-xs text-midnight/45">{popup.title}</p>
                  </div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                  <div className="rounded-lg bg-[#faf8f3] py-2">
                    ...
                    <p className="text-[9px] uppercase tracking-[0.15em] text-midnight/40">Views</p>
                  </div>
                  <div className="rounded-lg bg-[#faf8f3] py-2">
                    <p className="text-sm font-semibold text-midnight">{popup.clicks || 0}</p>
                    <p className="text-[9px] uppercase tracking-[0.15em] text-midnight/40">Clicks</p>
                  </div>
                  <div className="rounded-lg bg-[#faf8f3] py-2">
                    <p className="text-sm font-semibold text-midnight">{ctr.toFixed(1)}%</p>
                    <p className="text-[9px] uppercase tracking-[0.15em] text-midnight/40">CTR</p>
                  </div>
                </div>

                <div className="mt-4 flex items-center justify-between border-t border-midnight/5 pt-3 text-xs">
                  ...
                  <button type="button" onClick={() => remove(popup.id)} className="text-cardinal-500 hover:text-cardinal-700">
                    Delete
                  </button>
                </div>
              </div>
